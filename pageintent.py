"""
page_intent_tagger.py
---------------------
Reads the per-page JSON produced by the cleaner and adds a `page_intent`
field to every page object.

What is page_intent?
--------------------
Instead of detecting markdown sections within a page (the old approach),
this file answers one question per page:
    "What is the PRIMARY purpose of this page in the annual report?"

The intent is a single, human-readable label chosen from a fixed taxonomy
(see PAGE_INTENT_TAXONOMY below). It is the first thing a query router,
retrieval filter, or analyst would want to know about any page.

Design
------
* Heuristic-first: heading keywords + body keywords + structural signals
  (has_table, is_short, word_count, presence of ₹/numbers).
  No LLM call — deterministic, free, runs on every page in < 1ms.
* A confidence score (0.0–1.0) is also attached so the agent can decide
  when to trust the label and when to double-check.
* Priority order matters: more specific intents are tested before generic
  ones. First match wins.
* Every page gets an intent — nothing silently drops.

Output
------
The tagger writes a new JSON file where each page object has two extra
fields added:
    "page_intent"       : str   — canonical label from taxonomy
    "intent_confidence" : float — 0.0 to 1.0

Usage (standalone):
    python page_intent_tagger.py \\
        --input  KALYAN_ANNUAL_CLEANEDANDCLASSIFIED_2025.json \\
        --output KALYAN_ANNUAL_TAGGED_2025.json

Usage (as a library):
    from page_intent_tagger import PageIntentTagger
    tagger  = PageIntentTagger()
    pages   = tagger.tag_all(pages_list)   # adds page_intent in place
"""
